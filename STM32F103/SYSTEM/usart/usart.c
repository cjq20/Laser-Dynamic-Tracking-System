...
void USART2_Init(u32 bound)
{
	...
	USART_ITConfig(USART2, USART_IT_RXNE, ENABLE);//?a???à1??D??
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
		...
		if((USART_RX_STA&0x8000)==0)//接收未完成
			...
			if(USART_RX_STA&0x4000)//接收到了0x0d
				{
				if(Res!='K')USART_RX_STA=0;//接收错误,重新开始
				else USART_RX_STA|=0x8000;	//接收完成了 
				}
			else //还没收到0X0D
				{	
				if(Res=='D')USART_RX_STA|=0x4000;
				else
					{
	...
	if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
		...
		if((USART_RX_STA2&0x8000)==0)//接收未完成
			...
			if(USART_RX_STA2&0x4000)//接收到了0x0d
				{
				if(Res!='K')USART_RX_STA2=0;//接收错误,重新开始
				else USART_RX_STA2|=0x8000;	//接收完成了 
				}
			else //还没收到0X0D
				{	
				if(Res=='D')USART_RX_STA2|=0x4000;
				else
					{
